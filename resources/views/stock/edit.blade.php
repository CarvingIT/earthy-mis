    ...
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900">Edit Stock Transaction</h2>
            </div>
            <p class="max-w-xl text-sm font-medium text-slate-500">Update stock transaction details and correct any information.</p>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-4xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="form-panel reveal rounded-2xl p-6 sm:p-8">
                <div class="mb-6 flex items-start gap-4">
                    ...
                    <div>
                        <h3 class="text-xl font-extrabold text-slate-900">Edit transaction</h3>
                        <p class="mt-1 text-sm font-medium text-slate-500">Update the stock transaction details below.</p>
                    </div>
                </div>

                <form method="POST" action="{{ route('stock.update', $stock) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="Date" name="Date" type="date" class="form-input mt-1 block w-full" :value="old('Date', $stock->Date)" required />
                            <p class="mt-1 text-xs font-medium text-slate-500">When did this stock transaction occur?</p>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs font-medium text-slate-500">Which product is this transaction for?</p>
                            <x-input-error class="mt-2" :messages="$errors->get('product_id')" />
                        </div>
                    </div>

                    <div class="info-card rounded-xl p-4" id="product-info">
                        <div class="flex items-start gap-3">
                            <svg class="h-5 w-5 shrink-0 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <div class="flex-1">
                                <p class="text-sm font-bold text-blue-900">Product Unit Information</p>
                                ...
                                <p class="mt-2 text-xs font-semibold text-blue-700" id="unit-instruction">All quantities are tracked in the base unit.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="quantity" name="quantity" type="number" min="0.01" step="0.01" class="form-input mt-1 block w-full" :value="old('quantity', $stock->quantity)" placeholder="Enter quantity" required />
                            <p class="mt-1 text-xs font-medium text-slate-500" id="quantity-help">Transaction quantity.</p>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <select name="transaction_type" id="transaction_type" class="form-select mt-1 block w-full" required>
                                ...
                                <option value="purchase" {{ old('transaction_type', $stock->transaction_type) == 'purchase' ? 'selected' : '' }}>Purchase (New Stock)</option>
                                <option value="opening_stock" {{ old('transaction_type', $stock->transaction_type) == 'opening_stock' ? 'selected' : '' }}>Opening Stock (Initial)</option>
                                <option value="return" {{ old('transaction_type', $stock->transaction_type) == 'return' ? 'selected' : '' }}>Return (Customer Return)</option>
                                <option value="adjustment" {{ old('transaction_type', $stock->transaction_type) == 'adjustment' ? 'selected' : '' }}>Adjustment (Correction)</option>
                            </select>
                            <p class="mt-1 text-xs font-medium text-slate-500">What type of stock transaction is this?</p>
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3" class="form-input mt-1 block w-full" placeholder="Add any additional information about this transaction...">{{ old('notes', $stock->notes) }}</textarea>
                    ...
                    function updateLabels() {
                        ...
                        if (productSelect.value) {
                            ...
                            if (unitType === 'sales') {
                                ...
                                if (quantityHelp) {
                                    quantityHelp.textContent = `Transaction quantity in ${salesUnit}.`;
                                }
                                if (unitTypeHelp) {
                                ...
                                if (quantityHelp) {
                                    quantityHelp.textContent = `Transaction quantity in ${baseUnit}.`;
                                }
                                if (unitTypeHelp) {

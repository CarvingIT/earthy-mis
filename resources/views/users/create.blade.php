    ...
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900">Create User</h2>
            </div>
            <p class="max-w-xl text-sm font-medium text-slate-500">Create a staff account with secure credentials and the right permission level.</p>
        </div>
    </x-slot>
            ...
            <section class="form-panel reveal rounded-2xl p-5">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-extrabold text-slate-900">New staff account</h3>
                            <p class="text-sm font-medium text-slate-500">Set identity, login details, and admin access in one pass.</p>
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('users.store') }}">
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_20rem]">
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="name" name="name" type="text" class="form-field mt-2 block w-full" :value="old('name')" required autofocus placeholder="Full name" />
                                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                                    </div>

                                    <div>
                                        <label for="email" class="form-label">Email <span class="text-rose-500">*</span></label>
                                        <x-text-input id="email" name="email" type="email" class="form-field mt-2 block w-full" :value="old('email')" required placeholder="name@example.com" />
                            ...
                            <div class="relative z-10">
                                <div class="mb-5 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-medium text-slate-500">Password requirements and permission level.</p>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="password" name="password" type="password" class="form-field mt-2 block w-full" required placeholder="Minimum 8 characters" />
                                        <x-input-error class="mt-2" :messages="$errors->get('password')" />
                                    </div>

                                    <div>
                                        <label for="password_confirmation" class="form-label">Confirm password <span class="text-rose-500">*</span></label>
                                        <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="form-field mt-2 block w-full" required placeholder="Repeat password" />
                                    </div>
                                    ...
                                    <div class="md:col-span-2">
                                        <label for="is_admin" class="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 transition hover:border-emerald-200 hover:bg-emerald-50/50">
                                            <input id="is_admin" name="is_admin" type="checkbox" value="1" class="mt-1 rounded border-slate-300 text-emerald-600 shadow-sm focus:ring-emerald-500" @checked(old('is_admin'))>
                                            <span>
                                                <span class="block text-sm font-extrabold text-slate-900">Grant admin access</span>
                    ...
                    <aside class="form-aside reveal h-fit rounded-2xl p-5" style="--reveal-delay: 160ms;">
                        ...
                        <p class="mt-2 text-sm font-medium leading-6 text-slate-500">Name, email, and password are required. Admin access can be enabled now or adjusted later.</p>

    ...
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900">Edit Product</h2>
            </div>
            <p class="max-w-xl text-sm font-medium text-slate-500">Keep catalog identity, pricing, unit setup, and product notes accurate for daily operations.</p>
        </div>
    </x-slot>
            ...
            <section class="form-panel reveal rounded-2xl p-5">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-extrabold text-slate-900">{{ $product->name }}</h3>
                            <p class="text-sm font-medium text-slate-500">Update details and keep the product catalog reliable.</p>
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('products.update', $product) }}">
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_20rem]">
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="name" name="name" type="text" class="form-field mt-2 block w-full" :value="old('name', $product->name)" required autofocus />
                                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                                    </div>

                                    <div>
                                        <label for="sku" class="form-label">SKU</label>
                                        <x-text-input id="sku" name="sku" type="text" class="form-field mt-2 block w-full" :value="old('sku', $product->sku)" placeholder="SKU-001" />
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="price" name="price" type="number" step="0.01" min="0" class="form-field mt-2 block w-full" :value="old('price', $product->price)" required />
                                    ...
                                    <div>
                                        ...
                                        <select id="sales_unit_id" name="sales_unit_id" class="form-field mt-2 block w-full px-4 py-2.5 shadow-sm">
                                            ...
                                            @foreach($units as $unit)
                                                <option value="{{ $unit->id }}" {{ old('sales_unit_id', $product->sales_unit_id) == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>
                                            @endforeach
                                        </select>
                        ...
                        <section class="form-section reveal rounded-2xl p-5 sm:p-6" style="--reveal-delay: 150ms;">
                            <div class="relative z-10">
                                ...
                                <div>
                                    ...
                                    <textarea id="description" name="description" rows="4" class="form-field mt-2 block w-full px-4 py-3 shadow-sm" placeholder="Product details, usage notes, or catalog description">{{ old('description', $product->description) }}</textarea>
                    ...
                    <aside class="form-aside reveal h-fit rounded-2xl p-5" style="--reveal-delay: 170ms;">
                        ...
                        <p class="mt-2 text-sm font-medium leading-6 text-slate-500">Review catalog identity, price, base unit, sales unit, and description before updating this product.</p>

                        <div class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-extrabold uppercase tracking-wide text-slate-500">Current record</p>
                            <p class="mt-2 text-sm font-black text-slate-900">{{ $product->name }}</p>
                            <p class="mt-1 text-xs font-semibold text-slate-500">Rs. {{ number_format((float) $product->price, 2) }}{{ $product->baseUnit ? ' / ' . $product->baseUnit->name : '' }}</p>
                        </div>

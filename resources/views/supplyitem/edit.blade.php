<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900">Edit Supply Item</h2>
            </div>
            <p class="max-w-xl text-sm font-medium text-slate-500">Update supply transaction details and correct any information.</p>
        </div>
    </x-slot>
    ...
    <div class="supply-form-shell min-h-screen py-10">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <section class="form-panel reveal rounded-2xl p-5">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-extrabold text-slate-900">Edit supply transaction</h3>
                            <p class="text-sm font-medium text-slate-500">Update supply transaction details and correct any information.</p>
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('supplyitems.update', $supplyitem) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_20rem]">
                    <div class="space-y-6">
                        ...
                        <section class="form-panel reveal rounded-2xl p-6 sm:p-8" style="--reveal-delay: 70ms;">
                            <div class="mb-6">
                                <h3 class="text-xl font-extrabold text-slate-900">Supply Transaction Details</h3>
                                <p class="mt-1 text-sm font-medium text-slate-500">Update the supply transaction details below.</p>
                            </div>
                            ...
                            <div class="space-y-5">
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="Date" name="Date" type="date" class="form-field mt-2 block w-full" :value="old('Date', $supplyitem->Date)" required />
                                        <p class="mt-1 text-xs font-medium text-slate-500">When did this purchase occur?</p>
                                    ...
                                    <div>
                                        ...
                                        <select name="consumable_id" id="consumable_id" class="form-field mt-2 block w-full px-4 py-2.5 shadow-sm" required>
                                            ...
                                            @foreach($consumables as $item)
                                                <option value="{{ $item->id }}" {{ old('consumable_id', $supplyitem->consumable_id) == $item->id ? 'selected' : '' }}>
                                                    {{ $item->item }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="cost" name="cost" type="number" step="0.01" min="0" class="form-field mt-2 block w-full" :value="old('cost', $supplyitem->cost)" placeholder="e.g., 1500.00" required />
                            ...
                            <div class="space-y-5">
                                <div>
                                    ...
                                    <textarea id="description" name="description" rows="3" class="form-field mt-2 block w-full px-4 py-3 shadow-sm" placeholder="Examples: Supplier name, purpose, quality notes...">{{ old('description', $supplyitem->description) }}</textarea>

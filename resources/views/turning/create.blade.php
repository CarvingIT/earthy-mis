<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create Turning</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('turning.store') }}" class="space-y-4">
                    @csrf

                    <div>
                        <x-input-label for="windrow" value="Windrow Number" />
                        <select name="windrow_id" class="mt-1 block w-full">
                            <option value="">Select Windrow Number</option>
                            @foreach($windrow as $win)
                                <option value="{{ $win->id }}">{{ $win->windrow_number }}</option>
                            @endforeach
                        </select>

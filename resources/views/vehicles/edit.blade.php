    ...
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-3xl font-extrabold leading-tight text-slate-900">Edit Vehicle</h2>
            </div>
            <p class="max-w-xl text-sm font-medium text-slate-500">Keep registration, vehicle identity, and purchase information accurate for daily operations.</p>
        </div>
    </x-slot>
            ...
            <section class="form-panel reveal rounded-2xl p-5">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-extrabold text-slate-900">{{ $vehicle->registration_number }}</h3>
                            <p class="text-sm font-medium text-slate-500">Update details and keep the fleet directory reliable.</p>
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('vehicles.update', $vehicle) }}">
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_20rem]">
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="registration_number" name="registration_number" type="text" class="form-field mt-2 block w-full" :value="old('registration_number', $vehicle->registration_number)" required autofocus />
                                        <x-input-error class="mt-2" :messages="$errors->get('registration_number')" />
                                    </div>

                                    <div>
                                        <label for="type" class="form-label">Type</label>
                                        <x-text-input id="type" name="type" type="text" class="form-field mt-2 block w-full" :value="old('type', $vehicle->type)" placeholder="Truck, van, bike" />
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <x-text-input id="brand" name="brand" type="text" class="form-field mt-2 block w-full" :value="old('brand', $vehicle->brand)" placeholder="Tata" />
                                        <x-input-error class="mt-2" :messages="$errors->get('brand')" />
                                    </div>

                                    <div>
                                        <label for="model" class="form-label">Model</label>
                                        <x-text-input id="model" name="model" type="text" class="form-field mt-2 block w-full" :value="old('model', $vehicle->model)" placeholder="Ace" />
                                        <x-input-error class="mt-2" :messages="$errors->get('model')" />
                                    </div>

                                    <div>
                                        <label for="color" class="form-label">Color</label>
                                        <x-text-input id="color" name="color" type="text" class="form-field mt-2 block w-full" :value="old('color', $vehicle->color)" placeholder="White" />
                                        <x-input-error class="mt-2" :messages="$errors->get('color')" />
                                    </div>

                                    <div>
                                        <label for="purchased_on" class="form-label">Purchased on</label>
                                        <x-text-input id="purchased_on" name="purchased_on" type="date" class="form-field mt-2 block w-full" :value="old('purchased_on', $vehicle->purchased_on)" />
                    ...
                    <aside class="form-aside reveal h-fit rounded-2xl p-5" style="--reveal-delay: 160ms;">
                        ...
                        <p class="mt-2 text-sm font-medium leading-6 text-slate-500">Review registration, identity, and purchase information before updating this fleet record.</p>

                        <div class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
                            <p class="text-xs font-extrabold uppercase tracking-wide text-slate-500">Current record</p>
                            <p class="mt-2 text-sm font-black text-slate-900">{{ $vehicle->registration_number }}</p>
                            <p class="mt-1 text-xs font-semibold text-slate-500">{{ $vehicle->brand ?: 'Brand not set' }}{{ $vehicle->model ? ' / ' . $vehicle->model : '' }}</p>
                        </div>
